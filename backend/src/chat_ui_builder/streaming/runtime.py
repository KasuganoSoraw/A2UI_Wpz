...
class StreamingSessionState(BaseModel):
    ...
    binding_state_summary: dict[str, Any] = Field(
        default_factory=lambda: {"bindings": []}
    )
...
class StreamingRuntime:
        ...
        *,
        ...
        async with lock:
            ...
            if state.is_processing:
                logger.info(
                    "runtime skip start: session is processing session_id=%s",
                    session_id,
                )
                yield {
                    "type": "status",
    ...
    async def _drain_session(
        self, state: StreamingSessionState
    ) -> AsyncIterator[dict[str, Any]]:
        ...
        while True:
            ...
            logger.info(
                "runtime trigger segment session_id=%s segment_id=%s",
                state.session_id,
                segment_id,
            )
            ...
            if final_item is None:
                logger.warning(
                    "runtime missing final item session_id=%s segment_id=%s",
                    state.session_id,
                    segment_id,
                )
                continue
            ...
            async with lock:
                ...
                state.binding_state_summary = final_item.get(
                    "binding_state_summary", state.binding_state_summary
                )
                state.page_state_summary = final_item.get(
                    "page_state_summary", state.page_state_summary
                )
    ...
    def _should_trigger_incremental_render(
        self, *, extraction: JsonExtractionResult
    ) -> bool:
        if self._has_array_growth(extraction.changes, threshold=2):
            return True

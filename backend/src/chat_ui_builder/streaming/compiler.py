...
class StreamBlockState:
    ...
    dataset_id: str | None
    block_type: Literal[
        "text", "facts", "list", "table", "summary_text", "summary_facts"
    ]
    title: str | None = None
...
class StreamCompiler:
    ...
    def _ensure_surface_initialized(self) -> None:
        if not self.surface_initialized:
            raise ValueError(
                "Stream surface 尚未初始化，请先发送 init_stream_surface 事件。"
            )
    ...
    def _register_dataset_binding(self, dataset_id: str, block_id: str) -> None:
        ...
        if existing and existing != block_id:
            raise ValueError(
                f"dataset_id={dataset_id} 已绑定到 block={existing}，不能再绑定到 {block_id}。"
            )
    ...
    def _require_block(
        self, block_id: str, expected_type: str | None = None
    ) -> StreamBlockState:
        ...
        if expected_type and state.block_type != expected_type:
            raise ValueError(
                f"block_id={block_id} 类型是 {state.block_type}，与期望 {expected_type} 不匹配。"
            )
        return state
    ...
    def _emit_text_lines(
        self, block_id: str, lines: list[TextLine], start_index: int = 0
    ) -> list[A2UIFrame]:
    ...
    def _handle_init_stream_surface(
        self, event: InitStreamSurfaceEvent
    ) -> list[A2UIFrame]:
    ...
    def _handle_create_text_block(self, event: CreateTextBlockEvent) -> list[A2UIFrame]:
        ...
        frames.extend(
            self._emit_text_lines(
                event.block_id, event.lines, start_index=state.text_line_count
            )
        )
    ...
    def _handle_append_text_lines(self, event: AppendTextLinesEvent) -> list[A2UIFrame]:
        ...
        frames = self._emit_text_lines(
            event.block_id, event.lines, start_index=state.text_line_count
        )
        state.text_line_count += len(event.lines)
        return frames

    def _handle_create_facts_block(
        self, event: CreateFactsBlockEvent
    ) -> list[A2UIFrame]:
        self._ensure_surface_initialized()
        self._register_dataset_binding(event.dataset_id, event.block_id)
    ...
    def _handle_add_list_items(self, event: AddListItemsEvent) -> list[A2UIFrame]:
        ...
        if not state.list_host_id:
            raise ValueError(
                f"block_id={event.block_id} 缺少 list_host_id，无法 add_list_items。"
            )
        return self._emit_list_items(state.list_host_id, event.items)

    def _handle_create_table_block(
        self, event: CreateTableBlockEvent
    ) -> list[A2UIFrame]:
        self._ensure_surface_initialized()
        self._register_dataset_binding(event.dataset_id, event.block_id)
        self._register_block(
    ...
    def _handle_append_table_rows(self, event: AppendTableRowsEvent) -> list[A2UIFrame]:
        ...
        if cache is None:
            raise ValueError(
                f"block_id={event.block_id} 缺少 table cache，无法 append_table_rows。"
            )
    ...
    def _handle_set_final_summary_text(
        self, event: SetFinalSummaryTextEvent
    ) -> list[A2UIFrame]:
        self._ensure_surface_initialized()
        ...
        if state.block_type != "summary_text":
            raise ValueError(
                f"block_id={event.block_id} 已存在且类型为 {state.block_type}，不能设置 final summary text。"
            )
    ...
    def _handle_set_final_summary_facts(
        self, event: SetFinalSummaryFactsEvent
    ) -> list[A2UIFrame]:
        self._ensure_surface_initialized()
        ...
        if state.block_type != "summary_facts":
            raise ValueError(
                f"block_id={event.block_id} 已存在且类型为 {state.block_type}，不能设置 final summary facts。"
            )

...
class RegionArchetypeBuilder:
    ...
    def build(
        self, context: RegionBuildContext, emit: EmitLowLevel
    ) -> RegionBuildResult:
        raise NotImplementedError

    def _create_region_root(
        self,
        context: RegionBuildContext,
        emit: EmitLowLevel,
        *,
        layout: str = "Column",
        appearance="region_in_root",
    ) -> list[A2UIFrame]:
        return emit(
            AddSectionDelta(
                event="add_section",
    ...
    def _create_header_and_body(
        ...
        *,
        ...
        if include_body_slot:
            body_layout = (
                "Column" if context.arrangement.body_layout == "column" else "Row"
            )
    ...
    def _default_slot_parents(
        self, region_id: str, content_parent: str
    ) -> dict[str, str]:
...
class HeroArchetypeBuilder(RegionArchetypeBuilder):
    ...
    def build(
        ...
        return RegionBuildResult(
            archetype=self.archetype_name, frames=frames, slot_parents=slot_parents
        )
...
class DetailsArchetypeBuilder(RegionArchetypeBuilder):
    ...
    def build(
        ...
        frames = self._create_region_root(context, emit)
        header_body_frames, content_parent = self._create_header_and_body(
            context, emit, include_body_slot=True
        )
        frames.extend(header_body_frames)
...
class WorkflowArchetypeBuilder(RegionArchetypeBuilder):
    ...
    def build(
        ...
        frames.extend(
            self._create_section(
                emit, section_id=flow_id, parent_id=region_id, layout="Column", order=30
            )
        )
        slot_parents = self._default_slot_parents(region_id, content_parent)
...
class ListArchetypeBuilder(RegionArchetypeBuilder):
    ...
    def build(
        ...
        frames = self._create_region_root(context, emit)
        header_body_frames, content_parent = self._create_header_and_body(
            context, emit, include_body_slot=False
        )
        frames.extend(header_body_frames)

        list_items_id = f"{region_id}_list_items"
        list_layout = (
            "Timeline" if context.presentation_variant == "timeline" else "List"
        )
        frames.extend(
            self._create_section(
                emit,

...
class FrameCompiler:
    ...
    def apply(self, delta: Any) -> list[A2UIFrame]:
        logger.info(
            "Compiling delta type=%s payload=%s",
            type(delta).__name__,
            delta.model_dump(),
        )
    ...
    def _register_id(self, requested_id: str) -> str:
        ...
        logger.warning(
            "Duplicate component id detected: %s -> %s", requested_id, canonical
        )
    ...
    def _append_child(
        self, parent_id: str, child_id: str, order: int | None = None
    ) -> None:
        parent = self._ensure_container(parent_id)
        if parent.component_id == child_id:
            raise ValueError(
                f"Child id {child_id} cannot equal parent id {parent.component_id}"
            )
        canonical_parent_id = self._resolve_parent_id(parent_id)
        ...
        parent.child_ids.sort(
            key=lambda existing_id: self.local_child_order.get(
                (canonical_parent_id, existing_id), 10_000
            )
        )

    def _surface_update(self, components: list[ComponentNode]) -> A2UIFrame:
        frame = A2UIFrame(
            surfaceUpdate={"surfaceId": self.surface_id, "components": components}
        )
        logger.debug(
            "Compiled surfaceUpdate frame=%s", frame.model_dump(exclude_none=True)
        )
        return frame
    ...
    def _data_update(self, path: str, contents: list[DataMapEntry]) -> A2UIFrame:
        ...
        logger.debug(
            "Compiled dataModelUpdate frame=%s", frame.model_dump(exclude_none=True)
        )
        return frame
    ...
    def _init_surface(self, delta: InitSurfaceDelta) -> list[A2UIFrame]:
        ...
        self.containers = {
            self.root_id: ContainerState(
                component_id=self.root_id, container_type="Row"
            )
        }
        ...
        begin = A2UIFrame(
            beginRendering={
                "surfaceId": self.surface_id,
                "root": self.root_id,
                "styles": delta.theme.model_dump(exclude_none=True)
                if delta.theme
                else None,
            }
        )
        logger.debug(
            "Compiled beginRendering frame=%s", begin.model_dump(exclude_none=True)
        )
        return [begin, self._surface_update([root_component])]

    def _add_section(self, delta: AddSectionDelta) -> list[A2UIFrame]:
        actual_parent_id = (
            self.page_parent_id if delta.parent_id == self.root_id else delta.parent_id
        )
        parent = self._ensure_container(actual_parent_id)
        section_id = self._register_id(delta.id)
        if section_id == parent.component_id:
            raise ValueError(
                f"Section id {section_id} cannot equal parent id {parent.component_id}"
            )
    ...
    def _add_text(self, delta: AddTextDelta) -> list[A2UIFrame]:
        ...
        if text_id == parent.component_id:
            raise ValueError(
                f"Text id {text_id} cannot equal parent id {parent.component_id}"
            )
        ...
        text_component = ComponentNode(
            ...
            component={
                "Text": {
                    "text": {"path": f"/content/{text_id}/text"},
                    "usageHint": delta.usage_hint,
                }
            },
        )
        return [
            ...
            self._data_update(
                f"/content/{text_id}",
                [DataMapEntry(key="text", valueString=delta.text)],
            ),
        ]
    ...
    def _add_key_value(self, delta: AddKeyValueDelta) -> list[A2UIFrame]:
        ...
        if group_id == parent.component_id:
            raise ValueError(
                f"Key/value id {group_id} cannot equal parent id {parent.component_id}"
            )
        ...
        label = ComponentNode(
            ...
            component={
                "Text": {
                    "text": {"path": f"/content/{group_id}/label"},
                    "usageHint": "caption",
                }
            },
        )
        value = ComponentNode(
            ...
            component={
                "Text": {
                    "text": {"path": f"/content/{group_id}/value"},
                    "usageHint": "body",
                }
            },
        )
    ...
    def _add_image(self, delta: AddImageDelta) -> list[A2UIFrame]:
        ...
        if image_id == parent.component_id:
            raise ValueError(
                f"Image id {image_id} cannot equal parent id {parent.component_id}"
            )
        ...
        return [
            ...
            self._data_update(
                f"/content/{image_id}", [DataMapEntry(key="url", valueString=delta.url)]
            ),
        ]
    ...
    def _add_spec_component(
        ...
        *,
        ...
        if component_id == parent.component_id:
            raise ValueError(
                f"{component_name} id {component_id} cannot equal parent id {parent.component_id}"
            )
        ...
        spec_component = ComponentNode(
            id=component_id,
            component={
                component_name: {"spec": {"path": f"/content/{component_id}/spec"}}
            },
        )
    ...
    def _update_table_spec(self, delta: UpdateTableSpecDelta) -> list[A2UIFrame]:
        ...
        return [
            self._data_update(
                f"/content/{table_id}",
                [DataMapEntry(key="spec", valueString=delta.spec_json)],
            ),
        ]
    ...
    def _add_divider(self, delta: AddDividerDelta) -> list[A2UIFrame]:
        ...
        if divider_id == parent.component_id:
            raise ValueError(
                f"Divider id {divider_id} cannot equal parent id {parent.component_id}"
            )
        self._append_child(parent_id, divider_id)
        parent_update = self._container_component(parent)
        divider = ComponentNode(
            id=divider_id, component={"Divider": {"axis": "horizontal"}}
        )
    ...
    def _add_list_item(self, delta: AddListItemDelta) -> list[A2UIFrame]:
        ...
        if wrapper_id == parent.component_id:
            raise ValueError(
                f"List item wrapper id {wrapper_id} cannot equal parent id {parent.component_id}"
            )
        ...
        if timeline_card_id:
            item_surface_components.append(
                ComponentNode(
                    id=timeline_card_id, component={"Card": {"child": content_id}}
                )
            )
